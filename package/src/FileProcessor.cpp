...
void SDDS::FileProcessor::convertFile(std::string inputFile,
  ...
  if (fileContent.empty()) {
    ...
    const std::string HTMLContentStr =
        SDDS::generateHTMLAsStr(fileName, fileContent, stylesheet, language);
    if (outputFileDir.substr(0, 2) == "./" ||
        outputFileDir.substr(0, 2) ==
            ".\\") // remove the ./ or .\ at the begining of the output file
                   // directory.
      outputFileDir = outputFileDir.substr(outputFileDir.find(".") + 1,
                                           outputFileDir.length());
    if (outputFileDir.find("\\") == std::string::npos &&
        outputFileDir.find("/") ==
            std::string::npos) // Append the output path with "/" if the output
                               // arg is not postfixed with /
      outputFileDir.append("/");
    std::string filePath = "./" + outputFileDir + fileName + ".html";
    if (SDDS::writeHTMLToFile(filePath, HTMLContentStr))
      std::cout << filePath << " is successfully converted and written."
                << std::endl;
    else {
...
void SDDS::FileProcessor::processFile(std::string inputFile,
                                      std::string outputFileDir,
                                      std::string styleSheet,
                                      std::string language) {
  if (outputFileDir.empty()) {
    std::cout
        << "Default directory \"dist\" will be used as the output directory."
        << std::endl;
    outputFileDir = "dist/";
  }
  if (std::filesystem::exists(
          outputFileDir)) { // Clear the output directory first.
  ...
  if (SDDS::checkIfItsDirectory(inputFile)) { // if the input arg is a directory
    for (const auto &entry : std::filesystem::directory_iterator(inputFile)) {
      chooseFileType(entry.path().string(), outputFileDir, styleSheet,
                     language);
    }
  }
...
void SDDS::FileProcessor::parseJSONConfig(std::string configPath,
...
void SDDS::FileProcessor::convertFileMD(std::string inputFile,
                                        std::string outputFileDir,
                                        std::string stylesheet,
                                        std::string language) {
  ...
  if (fileContent.empty()) {
    ...
    if (outputFileDir.substr(0, 2) == "./" ||
        outputFileDir.substr(0, 2) ==
            ".\\") // remove the ./ or .\ at the begining of output file
                   // directory.
      outputFileDir = outputFileDir.substr(outputFileDir.find(".") + 1,
                                           outputFileDir.length());
    if (outputFileDir.find("\\") == std::string::npos &&
        outputFileDir.find("/") ==
            std::string::npos) // Append output path with "/" if the output arg
...
void SDDS::FileProcessor::chooseFileType(std::string inputFile,
                                         std::string outputFileDir,
                                         std::string styleSheet,
                                         std::string language) {
  if (inputFile.find(".txt") != std::string::npos) // checks if it is txt file
    convertFile(inputFile, outputFileDir, styleSheet, language);
  else if (inputFile.find(".md") !=
           std::string::npos) // checks if it is md file
